...
func NewRootCmd(
	...
	buildApp AppBuilder,
	options ...Option,
) (*cobra.Command, bonusblockparams.EncodingConfig) {
	rootOptions := newRootOptions(options...)

	// Set config for prefixes
	SetPrefixes(accountAddressPrefix)

	encodingConfig := bonusblockparams.MakeEncodingConfig(moduleBasics)
...
func queryCommand(moduleBasics module.BasicManager) *cobra.Command {
	...
	cmd.PersistentFlags().String(flags.FlagChainID, "blocktopia-01", "The network chain ID")
